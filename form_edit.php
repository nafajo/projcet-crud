...
// tidak perlu pakai perulangan while, karena hanya ada 1 record
        ...
        $nama = $data["nama"];
        $tempat_lahir = $data["tempat_lhr"];
        $fakultas = $data["fakultas"];
        $jurusan = $data["jurusan"];
        $ipk = $data["ipk"];
// untuk tanggal harus dipecah
        $tgl = substr($data["tanggal_lhr"],8,2);
        $bln = substr($data["tanggal_lhr"],5,2);
        $thn = substr($data["tanggal_lhr"],0,4);
// bebaskan memory
...
//buat dan jalankan query UPDATE
        ...
        $query .= "nama = '$nama', tempat_lhr = '$tempat_lahir', ";
        $query .= "tanggal_lhr = '$tgl_lhr', fakultas='$fakultas', ";
...
// INSERT berhasil, redirect ke tampil_mahasiswa.php + pesan
            $pesan = "Mahasiswa dengan nama = \"<b>$nama</b>\" sudah berhasil di u
pdate";
